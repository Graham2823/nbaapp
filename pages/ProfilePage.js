...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchDataForTeams = async () => {
      const promises = favoriteTeams.map((team) => axios.get(`https://nbaapp.vercel.app/api/getTeam?teamName=${team.teamName}`));
  ...
  useEffect(() => {
    const fetchDataForPlayers = async () => {
      const promises = favoritePlayers.map((player) => axios.get(`https://nbaapp.vercel.app/api/getFavoritePlayers?firstName=${player.playerName.split(" ")[0]}&lastName=${player.playerName.split(" ")[1]}${player.playerName.split(" ")[2]? "%20"+ player.playerName.split(" ")[2] : ''}`));
