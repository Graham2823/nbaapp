	...
	const handleFavoriteTeam = async () => {
		try {
			...
			const response = await axios.post(
				`https://nbaapp.vercel.app/api/favorites/addFavoriteTeam`,
				requestBody
			);
			...
			localStorage.setItem(
				'favoriteTeams',
				JSON.stringify(response.data.user.favoriteTeams)
			);
			if (
				response.data.user.favoriteTeams.some(
					(teams) => teams.teamName === teamDetails.teamName
				)
			) {
		...
		<div>
			<div
				...
				<h2>
					...
					favoriteTeams &&
					favoriteTeams.some(
						(team) => team.teamName === teamDetails.teamName
					) ? (
						<FontAwesomeIcon
							icon={faStar}
